...
# ponytail: single process-wide lock serializes conflict/quota checks and
# cancellation against concurrent requests; split into per-room/per-booking
# locks if throughput ever becomes a bottleneck.
...
def create_booking(
    ...
    with _booking_lock:
        ...
        reference.ensure_initialized(db)
        booking = Booking(
            room_id=room.id,
            user_id=user.id,
            start_time=start,
...
def cancel_booking(
    ...
    with _booking_lock:
        ...
        # half-cents round up, done in integer arithmetic to avoid float error
